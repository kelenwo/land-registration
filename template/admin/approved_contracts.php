...
<td><?php echo $req['bid_number']; ?></td>
  ...
  <td><?php if($req['has_finished']=="completed"): ?>
    ...
    <?php endif;?></td>
    <td class="actions">
      <button class="btn btn-success" id="save1-<?php echo $req['id'];?>" <?php if($req['bid_status']=="authorized"){echo 'disabled';}?>>Authorize
        <i id="loading1-<?php echo $req['id'];?>" class="fas fa-cog fa-spin"></i></button>
      <form id="update_contract-<?php echo $req['id'];?>">
        <input type="hidden" name="id" value="<?php echo $req['id'];?>">
        <input type="hidden" name="bid_by_email" value="<?php echo $req['bid_by_email'];?>">
        <input type="hidden" name="bid_status" value="authorized"></form>
    </td>
</tr>
<!-- List contract modal -->
...
<h6><?php echo $req['contract_title'];?></h6></div>
    ...
    <?php endif;?>
        ...
        <h6><?php echo $req['bid_amount'];?></h6></div>
  ...
  <h6><?php echo $req[0]['points'];?></h6></div>
    <?php if($req['contract_status']=="active"): ?>
...
<!-- General Information end -->
  ...
  <form id="update__contract_<?php echo $req['id'];?>">
    <input type="hidden" name="id" value="<?php echo $req['id'];?>">
    <input type="hidden" name="bid_by_email" value="<?php echo $req['bid_by_email'];?>">
    <input type="hidden" name="bid_number" value="<?php echo $req['bid_number'];?>">
    <input type="hidden" name="has_finished" value="completed">
...
<?php endif;?>
    ...
    <button class="btn pl-5 pr-5 btn-success" id="save-<?php echo $req['id'];?>" <?php if($req['bid_status']=="authorized"){echo 'disabled';}?>><i class="far fa-edit"></i> Authorize Contract &nbsp;
    <i id="loading-<?php echo $req['id'];?>" class="fas fa-cog fa-spin"></i></button>
    <?php endif;?>
